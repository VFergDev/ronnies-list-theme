...
class Like {
  constructor() {
    if (document.querySelector(".like-box")) {
      (axios__WEBPACK_IMPORTED_MODULE_0___default().defaults).headers.common["X-WP-Nonce"] = siteData.nonce;
      this.events();
    }
  }
  events() {
  ...
  async createLike(currentLikeBox) {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(siteData.root_url + "/wp-json/university/v1/manageLike", {
  ...
  async deleteLike(currentLikeBox) {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default()({
        url: siteData.root_url + "/wp-json/university/v1/manageLike",
        method: 'delete',
        data: {
...
class MyNotes {
  ...
  async deleteNote(e) {
    const thisNote = this.findNearestParentLi(e.target);
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default()["delete"](siteData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"));
  ...
  async updateNote(e) {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(siteData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"), ourUpdatedPost);
      this.makeNoteReadOnly(thisNote);
    } catch (e) {
      console.log("Sorry");
    }
  ...
  async createNote() {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(siteData.root_url + "/wp-json/wp/v2/note/", ourNewPost);
...
class Search {
  ...
  async getResults() {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(siteData.root_url + "/wp-json/university/v1/search?term=" + this.searchField.value);
      ...
      this.resultsDiv.innerHTML = `
        <div class="row">
          ...
          <div class="one-third">
            ...
            ${results.programs.length ? '<ul class="link-list min-list">' : `<p>No programs match that search. <a href="${siteData.root_url}/programs">View all programs</a></p>`}
          ...
          <div class="one-third">
            ...
            ${results.campuses.length ? '<ul class="link-list min-list">' : `<p>No campuses match that search. <a href="${siteData.root_url}/campuses">View all campuses</a></p>`}
              ${results.campuses.map(item => `<li><a href="${item.url}">${item.title}</a></li>`).join("")}
            ${results.campuses.length ? "</ul>" : ""}

            <h2 class="search-overlay__section-title">Events</h2>
            ${results.events.length ? "" : `<p>No events match that search. <a href="${siteData.root_url}/events">View all events</a></p>`}
