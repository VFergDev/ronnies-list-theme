...
class MyNotes {
  constructor() {
    if (document.querySelector("#my-notes")) {
      axios.defaults.headers.common["X-WP-Nonce"] = siteData.nonce
  ...
  async deleteNote(e) {
    ...
    try {
      const response = await axios.delete(siteData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"))
  ...
  async updateNote(e) {
    ...
    try {
      const response = await axios.post(siteData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"), ourUpdatedPost)
      this.makeNoteReadOnly(thisNote)
    } catch (e) {
      console.log("Sorry")
    }
  ...
  async createNote() {
    ...
    try {
      const response = await axios.post(siteData.root_url + "/wp-json/wp/v2/note/", ourNewPost)
